...
infile = open('sk_pickle_prob.pickle', 'rb')
roc_predictions = pickle.load(infile)
infile.close()
infile = open('labels_binary_sk', 'rb')
labels = pickle.load(infile)
infile.close()
# infile = open('ROC_data', 'rb')
# roc_data = pickle.load(infile)
# infile.close()
# infile = open('target_results', 'rb')
# target_results = pickle.load(infile)
# infile.close()
...
DNNV_predictions = np.array(mc12_predictions)
DNNV_predictions = mc12_predictions.T[0]
FLAT_predictions = np.array(mc12_predictions)
FLAT_predictions = mc12_predictions.T[0]
CATBOOST_predictions = np.array(mc12_predictions)
CATBOOST_predictions = mc12_predictions.T[0]
...
for x in reversed(range(len(probNN_labels))):
    if probNN_labels[x] == 2:
        ...
        DNNV_predictions = np.delete(mc15_predictions, x, 0)
        FLAT_predictions = np.delete(mc15_predictions, x, 0)
        CATBOOST_predictions = np.delete(mc15_predictions, x, 0)
        print(x)
print("step 2")
...
tpr_DNNV, fpr_DNNV, thresholds_DNNV = roc_curve(probNN_labels, mc15_predictions)
tpr_FLAT, fpr_FLAT, thresholds_FLAT = roc_curve(probNN_labels, mc15_predictions)
tpr_CATBOOST, fpr_CATBOOST, thresholds_CATBOOST = roc_curve(probNN_labels, mc15_predictions)
